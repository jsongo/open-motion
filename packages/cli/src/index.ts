  ...
  const files = {
    'package.json': JSON.stringify({
      ...
      scripts: {
        ...
        render: 'open-motion render -u http://localhost:5173 --composition main -o ./out.mp4',
        'render:fast': 'npm run build && (npx http-server dist -p 5173 > /dev/null 2>&1 & sleep 2 && npm run render -- \$@ && pkill -f http-server)',
